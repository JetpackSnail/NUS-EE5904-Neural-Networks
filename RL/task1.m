...
number_of_runs = 10;        % number of runs to do to find average
...
for run = 1:number_of_runs
    ...
    while grid_idx(idx1,idx2)~=final_state && reward_grid(idx1,idx2)==0
        ...
        [reward_grid,move_vec,qstates_vec,action_vec,idx1,idx2] = get_action_vec(reward_grid,action_o,idx1,idx2,action_vec,qstates_vec,grid_idx,final_m,final_n,move_vec,reward_function);
    end
    ...
    if r > max_reward
        max_reward = r;
        max_vecs = {reward_grid,move_vec,qstates_vec,action_vec};
    end
...
disp(['Number of reached runs is ', num2str(reached)])

disp(['Total average time for convergence is ' num2str(tot_avg_t)]);